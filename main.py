...
============================================================
Fix v17:
  - Rimozione filechooser (restituisce URI content:// su Android → inutilizzabile)
  - Popup selettore cartelle con path reali del filesystem
  - Scansione automatica cartelle Music comuni
  - Errori visibili in UI (non solo nel log)
"""
...
# ── Percorsi Music comuni su Android ─────────────────────────────────────────
def _find_music_dirs() -> list:
    """Cerca cartelle con MP3 nei percorsi standard Android."""
    roots = [
...
class AppState:
    ...
    def set_vol(self,v): self._volume=v; (self._sound and setattr(self._sound,"volume",v))
    ...
    def is_playing(self): return self._sound is not None and self._sound.state=="play"
    def position(self):
...
class SongRow(RecycleDataViewBehavior,BoxLayout):
    ...
    def __init__(self,**kw):
        ...
        self.spacing=dp(2); self.size_hint_y=None; self.height=dp(72); self._song=None
...
# =============================================================================
# Popup selettore cartelle — evita filechooser (restituisce content:// su Android)
# =============================================================================
class FolderPickerPopup(Popup):
    def __init__(self,on_folder_selected,**kw):
        ...
        content.add_widget(Label(
            text="Seleziona cartella da analizzare:",
        ...
        content.add_widget(Label(
            text="Oppure digita percorso manualmente:",
            font_size=sp(11),color=hc(C["subtext"]),size_hint_y=None,height=dp(22)))
        path_row=BoxLayout(size_hint_y=None,height=dp(42),spacing=dp(6))
        self._inp=TextInput(
            ...
            multiline=False,size_hint_x=0.72,padding=[dp(8),dp(10)])
        go=DkBtn(text="➤ Vai",size_hint_x=0.28,bg=hc(C["primary"]))
        go.bind(on_press=self._custom)
        path_row.add_widget(self._inp); path_row.add_widget(go); content.add_widget(path_row)

        cancel=DkBtn(text="✕ Annulla",size_hint_y=None,height=dp(38),bg=hc(C["card"]))
        cancel.bind(on_press=self.dismiss); content.add_widget(cancel)

        super().__init__(title="📂 Scegli cartella Music",content=content,
                         size_hint=(0.92,0.80),background_color=hc(C["surface"]),
                         title_color=hc(C["text"]),**kw)

        self._loading=Label(text="⏳ Cerco cartelle con MP3…",
    ...
    def _show(self,dirs):
        ...
        if not dirs:
            self._grid.add_widget(Label(
                text="Nessuna cartella con MP3 trovata.\n"
                     "Usa il campo sotto per inserire il percorso\n"
                     "oppure verifica i permessi storage.",
                font_size=sp(12),color=hc(C["subtext"]),
                size_hint_y=None,height=dp(66),halign="center"))
            return
        for path,count in dirs:
            row=BoxLayout(size_hint_y=None,height=dp(58),spacing=dp(8),padding=[dp(6),dp(4)])
            def _d(r,*_):
                r.canvas.before.clear()
                with r.canvas.before:
                    Color(*hc(C["card"])); RoundedRectangle(pos=r.pos,size=r.size,radius=[dp(8)])
            row.bind(pos=_d,size=_d)
            ...
            info.add_widget(Label(text=f"{count} MP3",font_size=sp(10),
                                  color=hc(C["subtext"]),halign="left",
                                  size_hint_y=None,height=dp(20)))
            btn=DkBtn(text="✔ Usa",size_hint_x=0.30,bg=hc(C["primary"]),font_size=sp(12))
...
class LibraryScreen(Screen):
    ...
    def _build(self):
        ...
        scan_btn.bind(on_press=lambda *_:FolderPickerPopup(on_folder_selected=self._do_scan).open())
        hdr.add_widget(self.search); hdr.add_widget(scan_btn); root.add_widget(hdr)
        ...
        for mood in MOODS:
            ...
            mrow.add_widget(b)
        root.add_widget(mrow); self.add_widget(root)
    ...
    def _do_scan(self,folder):
        ...
        if not os.path.isdir(folder):
            self._toast(
                ...
                "Verifica il percorso e che l'app\n"
                "abbia il permesso 'Accesso ai file'."); return
        n=_count_mp3(folder)
        if n==0:
            self._toast(f"Nessun MP3 trovato in:\n{folder}"); return

        self.status.text=f"⏳ Analisi {n} brani…"
        self.pbar.value=2
    ...
    def _done(self,songs,folder):
        ...
        if songs:
            ...
            self.status.text="⚠ 0 brani elaborati"
            self._toast(
                f"Analisi completata ma 0 brani.\n\n"
                "Possibili cause:\n"
                "• Permessi storage non concessi\n"
                "• File MP3 danneggiati\n\n"
                "Vai in Impostazioni → App → Music Mood\n"
                "→ Permessi → Consenti accesso ai file.")

    def _err(self,short,full):
        self.pbar.value=0
        self.status.text=f"❌ {short[:50]}"
    ...
    def _toast(self,msg):
        ...
        p=Popup(title="Info",content=c,size_hint=(0.88,None),height=dp(340),
...
class PlayerScreen(Screen):
    ...
    def _loop(self,*_): state.toggle_loop(); self.loop_b.set_bg(hc(C["success"] if state._loop else C["card"]))
    def _shuf(self,*_): state.toggle_shuffle(); self.shuf_b.set_bg(hc(C["success"] if state._shuffle else C["card"]))
...
class StatsScreen(Screen):
    ...
    def _card(self,txt):
        ...
        card.bind(pos=_d,size=_d); card.add_widget(lbl); self.grid.add_widget(card)
